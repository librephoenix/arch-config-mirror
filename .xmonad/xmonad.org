...
Config { font = "UbuntuMono-R 18"
       ...
       , commands = [
                    ...
                    , Run BatteryP ["BAT0"]
                      ...
                      "-l", "color02Normal", "-m", "color05Normal", "-h", "color03Normal"] 10
                    , Run Brightness
                      [ "-t", "<fc=color04Normal><fn=1>\xf5dd</fn> <percent>% </fc>", "--",
                        "-D", "amdgpu_bl1"
                      ] 2
                    , Run Volume "default" "Master"
                      [ "-t", "<status>", "--"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %battery% %bright%<action=`xdotool key Super_L+y`>%default:Master%</action>}<box color=colorBgBright width=0>%UnsafeStdinReader%</box>{<action=`xdotool key Super_L+c`>%date%</action> "
       }
}
